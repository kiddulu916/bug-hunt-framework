...
class ExploitationChainSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'chain_name', 'chain_description', 'step_number',
            ...
            'final_impact_description', 'executed_at', 'is_final_step',
            'success_percentage'
        ]
        read_only_fields = [
            'id', 'executed_at', 'is_final_step', 'success_percentage'
        ]
...
class VulnerabilitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'cvss_score', 'impact_description', 'affected_url',
            'affected_parameter', 'http_method', 'payload_used', 'request_data',
            'response_data', 'discovered_by_tool', 'discovery_method',
            ...
            'manually_verified', 'verification_notes', 'discovered_at',
            'updated_at', 'exploitation_chains', 'severity_score', 'has_evidence',
            'owasp_description'
        ]
        read_only_fields = [
    ...
    def validate_vulnerability_name(self, value):
        ...
        if len(value.strip()) < 3:
            raise serializers.ValidationError(
                "Vulnerability name must be at least 3 characters"
            )
        return value.strip()
    ...
    def validate_false_positive_likelihood(self, value):
        ...
        if not (0.0 <= value <= 1.0):
            raise serializers.ValidationError(
                "False positive likelihood must be between 0.0 and 1.0"
            )
        return value
    ...
    def validate_screenshot_paths(self, value):
        ...
        for path in value:
            if not isinstance(path, str) or len(path.strip()) == 0:
                raise serializers.ValidationError(
                    "Each screenshot path must be a non-empty string"
                )
...
class VulnerabilityCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'false_positive_likelihood', 'screenshot_paths',
            'additional_evidence', 'is_exploitable', 'exploitation_difficulty',
            'exploitation_notes', 'remediation_suggestion',
...
class VulnerabilitySummarySerializer(serializers.ModelSerializer):
    ...
    target_name = serializers.CharField(
        source='scan_session.target.target_name', read_only=True
    )
    scan_session_name = serializers.CharField(
        source='scan_session.session_name', read_only=True
    )
