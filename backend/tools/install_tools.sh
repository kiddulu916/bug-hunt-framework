#!/bin/bash
# install_tools.sh - Install all penetration testing tools for Bug Bounty Platform
# This script integrates with the Python tool framework in backend/tools/

set -e

echo "[*] Starting tool installation for Bug Bounty Automation Platform..."
...
# Create Python tool integration wrapper
cat > /opt/tools/python_tool_wrapper.py << 'EOF'
#!/usr/bin/env python3
"""
Python tool wrapper for integration with backend/tools framework
"""
import sys
import os
sys.path.append('/app')
...
try:
    ...
    from backend.tools.base import ToolConfig
    print("Django integration ready")
except Exception as e:
    print(f"Django integration unavailable: {e}")

EOF
chmod +x /opt/tools/python_tool_wrapper.py
...
# Create Django tool integration script
cat > /opt/tools/bin/django_tool_runner.py << 'EOF'
#!/usr/bin/env python3
"""
Django Tool Runner - Integration script for backend/tools framework
"""
import sys
import os
import django
...
try:
    ...
    from backend.tools.orchestrator import ToolOrchestrator
    from backend.tools.base import ToolConfig, get_tool, list_tools
    from backend.tools import initialize_tools

    def main():
        """Main entry point for Django tool integration"""
        if len(sys.argv) < 2:
            print("Available commands:")
            print("  list - List all available tools")
            print("  run <tool_name> <target> - Run a specific tool")
            print("  orchestrate <target> - Run full orchestration")
            sys.exit(1)
        ...
        if command == "list":
            ...
            config = ToolConfig(target=target, output_dir="/app/scan_results")
        ...
        elif command == "orchestrate" and len(sys.argv) >= 3:
            target = sys.argv[2]
            orchestrator = ToolOrchestrator()

            # Run basic recon workflow
            print(f"Running orchestrated scan for: {target}")
            # Implementation would depend on specific workflow needs
    ...
    print(f"Django tools framework not available: {e}")
    print("Running in standalone mode...")
EOF
chmod +x /opt/tools/bin/django_tool_runner.py

# Create installation summary
cat > /opt/tools/INSTALLATION_SUMMARY.txt << 'EOF'
PENETRATION TESTING TOOLS INSTALLATION SUMMARY
==============================================
INTEGRATED WITH BACKEND/TOOLS FRAMEWORK
